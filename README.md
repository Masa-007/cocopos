...
ゲストユーザー向けの確認用として、以下のデモアカウントを利用できます。

- メールアドレス: cocopos.demo@example.com
- パスワード: password
...
4. マイページで投稿を編集・削除・振り返りが可能
   -  投稿内容は月ごとに集計され、各箱の記録や気分の推移などをデータとして可視化
...
### 日記アプリではない理由
一般的な日記アプリは、出来事や気持ちを自由に書き残せる一方で、記録が悩みやネガティブな内容に偏りやすく、振り返ったときに気分が沈んでしまうことがあります。
cocopos は投稿を「未来宣言」「心の整理」「感謝」の3つの箱に分けることで、悩みを書き出しつつも、前向きな宣言や感謝の記録が一覧として同時に目に入る設計にしています。
